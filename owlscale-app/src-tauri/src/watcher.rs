...
pub fn start_watcher(
    ...
    std::thread::spawn(move || {
        ...
        if let Err(e) = watcher.watch(&owlscale_dir, RecursiveMode::Recursive) {
            eprintln!(
                ...
                Ok(Ok(event)) => {
                    ...
                    match read_workspace_state(&owlscale_dir) {
                        Ok(new_state) => {
                            ...
                            if let Err(e) = app_handle.emit("owlscale://state-changed", &new_state)
                            {
                            ...
                            // Rebuild tray menu with updated task list
                            crate::rebuild_tray_menu(&app_handle, &new_state);

                            // ── Notifications (respect config) ───────────
                            let notifications_on = app_handle
                                .try_state::<AppState>()
                                .and_then(|s| s.config.lock().ok().map(|c| c.notifications_enabled))
                                .unwrap_or(true);

                            if new_state.pending_review > 0 && notifications_on {
                                notify_new_returned(&app_handle, &new_state);
                            } else if new_state.pending_review == 0 {
